  ...
  public static void main(String[] args) throws SchedulerException {
    ...
    JobDetail simpleJobDetail =
        JobBuilder.newJob(SimpleJob.class) // load the job class and bind the HelloJob
            .withIdentity("simple-job1", "simple-group1") // arg1: task(job) name  arg2: group name
            .build();
    // 2.2 JobDetail : Use the JobBuilder
    JobDetail cronJobDetail =
        JobBuilder.newJob(SimpleJob.class) // load the job class and bind the HelloJob
            .withIdentity("cron-job1", "cron-group1") // arg1: task(job) name  arg2: group name
            .build();
    ...
    SimpleTrigger simpleTrigger =
        TriggerBuilder.newTrigger()
            .startNow()
            // .withIdentity( "simple-trigger1", "simple-group1") // arg1: trigger name  arg2: group
            // name [not same with job group]
            .withSchedule(
                SimpleScheduleBuilder.simpleSchedule().repeatSecondlyForever(5)) // 5 second exec
            .build();
    ...
    CronTrigger cronTrigger =
        TriggerBuilder.newTrigger()
            ...
            .withIdentity(
                "cron-trigger1",
                "cron-group1") // arg1: trigger name  arg2: group name [not same with job group]
