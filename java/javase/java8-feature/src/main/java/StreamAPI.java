 ...
 * @function 1. 创建 Stream 2. 中间操作 3. 终止操作
 */
public class StreamAPI {
  ...
  public void testMiddleOperation() {
    ...
    stream
        ...
        .limit(2)
        .forEach(System.out::println); // 找到两个满足添加之后的就停止运算
    ...
    Stream<Stream<Character>> streamStream2 =
        list.stream().map(StreamAPI::filterCharactor); // {{'a', 'a', 'a'}, {'b', 'b', 'b',} ...}
    streamStream2.forEach(sm -> sm.forEach(System.out::println));

    System.out.println("--------flatMap-------------");
    Stream<Character> characterStream =
        list.stream().flatMap(StreamAPI::filterCharactor); // {'a', 'a', 'a', 'b', 'b', 'b', ...}
