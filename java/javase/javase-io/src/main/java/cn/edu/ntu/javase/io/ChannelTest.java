...
public class ChannelTest {
  ...
  public void testCopyWithChannelAndBuffer() throws IOException {
    ...
    try {
      ...
      outChannel =
          FileChannel.open(
              Paths.get(TEST_DATA_PATH_NEW), StandardOpenOption.WRITE, StandardOpenOption.CREATE);
  ...
  public void testCopyWithChannel() throws Exception {
    ...
    //            buffer.clear();
    //        }
    MappedByteBuffer inMappedBuff =
        inChannel.map(
            FileChannel.MapMode.READ_ONLY,
            0,
            // 将Channel中的数据转换成Buffer，读取Buffer就是读取Channel中的数据【将Channel数据写入Buffer】
            inChannel.size());
    MappedByteBuffer outMappedBuff =
        outChannel.map(
            FileChannel.MapMode.READ_WRITE,
            0,
            // 将Channel中的数据转换成Buffer；向Buffer里写，就是向Channel中写【将Buffer中的数据写入Channel】
            inChannel.size());

    // 3.直接对缓冲区进行读写操作
    byte[] buffer = new byte[inMappedBuff.limit()];
    // 将缓冲区中的数据获取到buffer中
    inMappedBuff.get(buffer);
    // 将buffer中的数据存入缓冲区
    outMappedBuff.put(buffer);
