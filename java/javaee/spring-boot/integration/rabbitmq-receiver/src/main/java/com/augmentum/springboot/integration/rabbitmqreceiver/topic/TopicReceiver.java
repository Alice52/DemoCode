package com.augmentum.springboot.integration.rabbitmqreceiver.topic;
  ...
  @RabbitListener(
      bindings =
          @QueueBinding(
              ...
              exchange = @Exchange(name = "${exchange.topic}", durable = "true", type = "topic"),
              key = {"topicsKey.*", "topicsKey.#"}))
  @RabbitHandler
  public void process(Object content) {
    LOG.info(
        "[{}] Get message from {}, message detail: {} ", ExchangeTypes.TOPIC, TOPIC_QUEUE, content);
  }

  //  private static final String[] topicsRoutingKeys = new String[] {"topicsKey.*", "topicsKey.#"};
  //  @PostConstruct
  //  public void init() {
