package cn.edu.ntu.javaee.springboot.quartz.sample.controller;

import cn.edu.ntu.javaee.springboot.quartz.sample.common.ApiResponse;
...
public class JobController {
  ...
  public ResponseEntity<ApiResponse> addJob(@Valid JobForm form) {
    try {
      ...
      return new ResponseEntity<>(
          ApiResponse.msg(e.getMessage()), HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  public ResponseEntity<ApiResponse> jobList(Integer currentPage, Integer pageSize) {
    ...
    return ResponseEntity.ok(
        ApiResponse.ok(Dict.create().set("total", all.getTotal()).set("data", all.getList())));
  }

  @GetMapping("/available")
  public ResponseEntity<ApiResponse> availableJobs() {

    Map<String, String> map = jobService.listAllAvailableJobs();
