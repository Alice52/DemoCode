                    ...
                    .forEach(
                        a -> {
                          if (a instanceof GetMapping) {
                            methodInfo.setUri(((GetMapping) a).value()[0]);
                            methodInfo.setMethod(HttpMethod.GET);
                          } else if (a instanceof PostMapping) {
                            methodInfo.setUri(((PostMapping) a).value()[0]);
                            methodInfo.setMethod(HttpMethod.POST);

                          } else if (a instanceof DeleteMapping) {
                            methodInfo.setUri(((DeleteMapping) a).value()[0]);
                            methodInfo.setMethod(HttpMethod.DELETE);

                          } else if (a instanceof PutMapping) {
                            methodInfo.setUri(((PutMapping) a).value()[0]);
                    ...
                    .ifPresent(
                        y -> {
                          ...
                          ParameterizedType parameterizedType =
                              (ParameterizedType) x[finalI].getParameterizedType();
