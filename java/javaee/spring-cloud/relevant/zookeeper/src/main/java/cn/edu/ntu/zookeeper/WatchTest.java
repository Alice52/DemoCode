...
public class WatchTest {
  ...
  public static String getData(String path) throws Exception {
    ...
    Optional.ofNullable(zooKeeper.exists(path, false))
        .ifPresent(
            x -> {
              try {
                String dataValue = new String(zooKeeper.getData(path, false, new Stat()));
                LOG.info(dataValue);
                data[0] = dataValue;
              } catch (Exception e) {
                e.printStackTrace();
  ...
  public String get(String path) {
    ...
    try {
      Stat stat = Optional.ofNullable(zooKeeper).orElseGet(WatchTest::startZK).exists(path, false);
