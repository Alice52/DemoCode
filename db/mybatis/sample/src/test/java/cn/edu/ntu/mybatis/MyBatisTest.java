...
public class MyBatisTest {
    ...
    /**
     * 1. 根据xml配置文件（全局配置文件)创建一个SqlSessionFactory对象 有数据源一些运行环境信息 2. sql映射文件; 配置了每一个sql, 以及sql的封装规则等.
     * 3. 将sql映射文件注册在全局配置文件中 4. 写代码: 1). 根据全局配置文件得到SqlSessionFactory; 2). 使用sqlSession工厂,
